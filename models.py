...
class Discriminator(nn.Module):
    ...
    def forward(self, imgs, label, part=None):
        if type(imgs) is not list:
            # Fix interpolation to handle 4-channel images
            if imgs.size(1) == 4:  # 4-channel case
                imgs_resized_big = F.interpolate(imgs, size=self.im_size)
                imgs_resized_small = F.interpolate(imgs, size=128)
                imgs = [imgs_resized_big, imgs_resized_small]
            else:  # 3-channel case
